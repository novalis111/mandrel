...
import { useNamingSearch } from '../../stores/namingStore';
...
const NamingFilters: React.FC<NamingFiltersProps> = ({
  onSearch,
  loading = false
}) => {
  const [form] = Form.useForm();
  const { searchParams, updateSearchParam, clearFilters, isFiltered } = useNamingSearch();
  ...
  const typeOptions = [
    { label: 'Variable', value: 'variable', color: '#52c41a' },
    { label: 'Function', value: 'function', color: '#1890ff' },
    { label: 'Component', value: 'component', color: '#722ed1' },
    { label: 'Class', value: 'class', color: '#fa8c16' },
    { label: 'Interface', value: 'interface', color: '#13c2c2' },
    { label: 'Module', value: 'module', color: '#eb2f96' },
    { label: 'File', value: 'file', color: '#8c8c8c' },
  ];

  const statusOptions = [
    { label: 'Active', value: 'active', color: '#52c41a' },
    { label: 'Deprecated', value: 'deprecated', color: '#8c8c8c' },
    { label: 'Conflicted', value: 'conflicted', color: '#ff4d4f' },
      ...
      >
        ...
        <Form.Item
          ...
          label={
            <Space>
              <SearchOutlined />
              <Text>Search Names</Text>
            </Space>
          }
        >
          <Input
            placeholder="Search names, types, or context..."
            allowClear
            ...
            <div>
              ...
              <div style={{ marginTop: '8px' }}>
                <Space wrap size="small">
                  ...
                  {searchParams.type && (
                    <Tag color="purple" closable onClose={() => handleFieldChange('type', undefined)}>
                      Type: {typeOptions.find(t => t.value === searchParams.type)?.label}
                    </Tag>
                  )}
                  {searchParams.status && (
                  ...
                  {searchParams.project_id && (
                    <Tag color="orange" closable onClose={() => handleFieldChange('project_id', undefined)}>
                      Project: {projectOptions.find(p => p.value === searchParams.project_id)?.label}
                    </Tag>
                  )}
                  {searchParams.created_by && (
                    <Tag color="cyan" closable onClose={() => handleFieldChange('created_by', '')}>
