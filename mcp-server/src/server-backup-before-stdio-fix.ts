...
class AIDISServer {
  ...
  private backgroundServicesReady: boolean = false;
  private featureFlagsLoaded: boolean = false;
  ...
  async start(): Promise<void> {
    ...
    // 🚀 CRITICAL: Setup MCP transport IMMEDIATELY after process lock
    if (!SKIP_STDIO_TRANSPORT) {
      console.log('🔗 Setting up MCP transport IMMEDIATELY (before any heavy initialization)...');
      const transport = new StdioServerTransport();
      
      console.log('🤝 Connecting to MCP transport NOW...');
      await this.server.connect(transport);
      
      console.log('✅ MCP Server connected - starting background initialization...');
      
      // Start background initialization without blocking
      setImmediate(() => this.initializeInBackground().catch(err => 
        console.error('❌ Background initialization failed:', err)
      ));
      
      console.log('✅ AIDIS MCP Server is ready - handshake can complete immediately!');
      return; // Exit start() method - background will handle the rest
    ...
    try {
      // ORACLE FIX #2: Initialize database with retry and circuit breaker (ONLY if no MCP)
      ...
      if (!SKIP_STDIO_TRANSPORT) {
        ...
        console.log('✅ AIDIS MCP Server connected - starting background initialization...');
        
        // 🚀 CRITICAL: Start background initialization WITHOUT blocking MCP handshake
        setImmediate(() => this.initializeInBackground().catch(err => 
          console.error('❌ Background initialization failed:', err)
        ));
  ...
  /**
   * Background initialization that doesn't block MCP handshake
   */
  private async initializeInBackground(): Promise<void> {
    console.log('🔄 Starting background initialization...');
    
    try {
      // Load feature flags (was blocking main thread)
      console.log('🏁 Loading feature flags in background...');
      ...
      if (!SKIP_DATABASE) {
        console.log('🗄️  Initializing database in background...');
        await RetryHandler.executeWithRetry(async () => {
          await this.circuitBreaker.execute(async () => {
            const startTime = Date.now();
            await dbPool.initialize();
            this.dbHealthy = true;
            console.log(`✅ Database ready (${Date.now() - startTime}ms)`);
            
            // Notify Amp that context and project tools are now available
            this.notifyToolsChangedSafe();
          });
        });
      } else {
        this.dbHealthy = true;
        console.log('✅ Database skipped (AIDIS_SKIP_DATABASE=true)');
      }
      ...
      if (!SKIP_BACKGROUND_SERVICES) {
        ...
        try {
          // Initialize queue manager, git tracking, pattern detection, metrics etc.
          // These can fail without breaking core functionality
          const { getQueueManager } = await import('./services/queueManager.js');
          await getQueueManager().initialize();
          
          const { startGitTracking } = await import('./services/gitTracker.js');
          await startGitTracking();
          
          const { startPatternDetection } = await import('./services/patternDetector.js');  
          await startPatternDetection();
          
          const { startMetricsCollection } = await import('./services/metricsCollector.js');
          await startMetricsCollection();
          
          this.backgroundServicesReady = true;
          this.notifyToolsChangedSafe();
          console.log('✅ Background services ready');
          
        } catch (error) {
          console.warn('⚠️ Some background services failed:', error);
          // Continue - core functionality still works
        }
      }
      
      console.log('🚀 Background initialization completed - all systems operational');
      
    } catch (error) {
      console.error('❌ Background initialization failed:', error);
      // Don't crash the server - essential tools still work
    }
  }
  
  private notifyToolsChangedSafe(): void {
    try {
      this.server.notifyToolsChanged();
      console.log('🔄 Notified Amp about new tools available');
    } catch (error) {
      console.warn('⚠️ Failed to notify tools changed:', error);
    }
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  (async () => {
    try {
      ...
      // DO NOT await ensureFeatureFlags() - it blocks MCP handshake for 30+ seconds!
      // Feature flags will be loaded during background initialization
