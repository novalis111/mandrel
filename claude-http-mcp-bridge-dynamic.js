...
class PortManager {
  ...
  async discoverServicePort(serviceName) {
    try {
      ...
      if (isHealthy) {
        console.error(`✅ Discovered ${serviceName} on port ${serviceInfo.port}`);
        return serviceInfo.port;
      } else {
        console.error(`⚠️  Service ${serviceName} is stale, using default port`);
        return this.defaultPorts[serviceName] || 8080;
      }
    } catch (error) {
      console.error(`❌ Error discovering port for ${serviceName}:`, error.message);
      return this.defaultPorts[serviceName] || 8080;
    }
  }
...
async function initializeAidisConnection() {
  console.error('🔍 Discovering AIDIS MCP service port...');
  aidisPort = await portManager.discoverServicePort('aidis-mcp');
  console.error(`🎯 AIDIS MCP service discovered on port ${aidisPort}`);
}
...
async function callAidisHttp(toolName, args) {
  ...
  return new Promise((resolve, reject) => {
    ...
    console.error(`🔄 HTTP Call: ${toolName} (port ${aidisPort})`);
    ...
    req.on('error', async (err) => {
      ...
      if (err.code === 'ECONNREFUSED') {
        console.error(`🔄 Connection refused on port ${aidisPort}, rediscovering...`);
        aidisPort = await portManager.discoverServicePort('aidis-mcp');
...
async function main() {
  try {
    ...
    console.error('🚀 AIDIS HTTP-MCP Bridge v2.1.0 - Connected with dynamic port discovery');
    console.error(`📡 AIDIS MCP Service: localhost:${aidisPort}`);
    ...
    aidisPort = 8080;
    console.error(`⚠️  Using fallback port ${aidisPort}`);

    const transport = new StdioServerTransport();
    await server.connect(transport);
    console.error('🚀 AIDIS HTTP-MCP Bridge v2.1.0 - Started with fallback configuration');
  }
}
