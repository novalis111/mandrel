...
import { validationMiddleware, taskSchemas } from './middleware/validation.js';
...
class AIDISServer {
  ...
  private async executeMcpTool(toolName: string, args: any): Promise<any> {
    ...
    switch (toolName) {
      ...
      case 'task_update':
        ...
        const validatedTaskUpdateArgs = taskSchemas.update.parse(args);
        return await this.handleTaskUpdate(validatedTaskUpdateArgs);
          ...
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                assignedTo: {
                  type: 'string',
                  description: 'Agent ID or name to assign/reassign task to'
                },
                progress: {
                  type: 'number',
                  minimum: 0,
  ...
  private async handleTaskUpdate(args: any) {
    // Build update object from provided args
    const updates: any = {};
    ...
    await agentsHandler.updateTask(args.taskId, updates);
    ...
    let updateSummary = `📋 Task: ${args.taskId}\n`;
    if (args.status) updateSummary += `📊 Status: ${args.status} ${statusIcon}\n`;
    if (args.priority) updateSummary += `🏷️ Priority: ${args.priority} ${priorityIcon}\n`;
    if (args.assignedTo) updateSummary += `🤖 Assigned To: ${args.assignedTo}\n`;
    if (args.progress !== undefined) updateSummary += `📈 Progress: ${args.progress}%\n`;
