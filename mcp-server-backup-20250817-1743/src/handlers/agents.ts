...
export class AgentsHandler {
    ...
    async updateTask(
        ...
        try {
            ...
            if (updates.assignedTo !== undefined) {
                // Convert agent name to ID if needed
                let assignedToId = updates.assignedTo;
                if (updates.assignedTo && !this.isUUID(updates.assignedTo)) {
                    const agentResult = await client.query('SELECT id FROM agents WHERE name = $1', [updates.assignedTo]);
                    if (agentResult.rows.length > 0) {
                        assignedToId = agentResult.rows[0].id;
                        console.log(`🔍 Resolved agent "${updates.assignedTo}" to ID: ${assignedToId}`);
                    } else {
                        console.log(`⚠️  Agent "${updates.assignedTo}" not found, setting to null`);
                        assignedToId = null;
                    }
                }
            ...
            const query = `UPDATE agent_tasks SET ${updateFields.join(', ')} WHERE id = $${paramIndex}`;
    ...
    async updateTaskStatus(
        ...
        return this.updateTask(taskId, { status, assignedTo, metadata });
    }
