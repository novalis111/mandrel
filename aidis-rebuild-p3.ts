          ...
          contents: [
            {
              ...
              text: JSON.stringify({
                server: {
                  ...
                  version: '1.0.0',
                  phase: 3,
                  description: 'AIDIS Essential server with resources capability'
                },
                capabilities: {
                  tools: {
                    enabled: true,
        ...
        try {
          ...
          return {
            content: [
              { 
                type: 'text' as const, 
                text: `🟢 AIDIS Enhanced Status: OPERATIONAL (Phase 3)
...
🆕 NEW in Phase 3:
  • MCP Resources capability active
  • Status endpoint: aidis://status  
  • Config endpoint: aidis://config
...
💡 Usage: Run 'aidis_help' to see all tools, 'aidis_explain <toolname>' for detailed help` 
          ...
          return {
            content: [
              { 
                type: 'text' as const, 
                text: `🟡 AIDIS Enhanced Status: DEGRADED (Phase 3)\n\n❌ Database connection failed: ${error instanceof Error ? error.message : 'Unknown error'}\n\n🔧 9 enhanced tools + 2 resources still available (some may not function)` 
              }
            ]
          };
        }
      ...
      case 'aidis_help':
        return {
          content: [
            { 
              type: 'text' as const, 
              text: `🧭 AIDIS Enhanced Tools (Phase 3)
...
✨ **NEW: MCP Resources** (Phase 3)
  • Status endpoint: aidis://status (live server health JSON)
  • Config endpoint: aidis://config (server configuration JSON)
...
🌟 This is Phase 3 AIDIS with MCP resources capability.
...
console.log('AIDIS Enhanced server ready - 9 enhanced tools + resources capability loaded (Phase 3)');
